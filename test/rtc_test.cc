 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(::libavm_test::VideoSource *video,
    ...
    if (video->frame() == 0) {
      ...
      // AV1E_SET_ENABLE_TX64 is obsolete/not present in AVM, so it is commented out.
      // encoder->Control(AV2E_SET_ENABLE_TX64, 0);
      ...
      encoder->Control(AV2E_SET_ENABLE_TRELLIS_QUANT, 0);
      ...
      // AV1E_SET_ENABLE_TX64 is obsolete/not present in AVM, so it is commented out.
      // encoder->Control(AV2E_SET_ENABLE_TX64, 1);
    }
  }
...
TEST_P(RtcTestLarge, RtcTest) {
  ::libavm_test::Y4mVideoSource video_nonsc("niklas_1280_720_30.y4m", 0, 60);
